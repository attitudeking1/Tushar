...
@Tushar.on(events.NewMessage(pattern="^[!?/]lock ?(.*)"))
@is_admin
async def lock(event, perm):
    ...
    if "text" in input_str:
       ...
       await Tushar.edit_permissions(event.chat_id,
          send_messages=False, 
          send_media=False,
          send_stickers=False,
          send_gifs=False,
...
@Tushar.on(events.NewMessage(pattern="^[!?/]unlock ?(.*)"))
@is_admin
...
@Tushar.on(events.NewMessage(pattern="^[!?/]locktypes"))
...
@Tushar.on(events.callbackquery.CallbackQuery(data="locks"))
async def _(event):

    await event.edit(LOCKS_HELP, buttons=[[Button.inline("« Back", data="help")]])
