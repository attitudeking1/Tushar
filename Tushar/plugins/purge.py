...
@Tushar.on(events.NewMessage(pattern=r"^[?!]purge"))
@is_admin
...
@Tushar.on(events.NewMessage(pattern="^[!?/]spurge"))
@is_admin
...
@Tushar.on(events.NewMessage(pattern="^[!?/]del$"))
@is_admin
...
@Tushar.on(events.callbackquery.CallbackQuery(data="purges"))
async def _(event):
    await event.edit(PR_HELP, buttons=[[Button.inline("« Back", data="help")]])
